...
function v() {
	$EDITOR ~/.zshrc
	source ~/.zshrc
	onOSX && cp ~/.zshrc ~/Documents/repos/tarikkochan.github.io/1.zshrc && cd ~/Documents/repos/tarikkochan.github.io/ && git add . && git commit -a -m "update 1.zshrc" && git push
}
onOSX && {
	function o() {
		[ -z "$1" ] && open . || open "$1"
	...
	function sleepafter() {
		...
		[ "$1" -lt 0 ] && echo "定时睡眠操作需要一个有效的秒数。" && return 1
		ds1
		echo -n $1
		sleep 1
		for ((i = 2; i <= $1; i++)); do
